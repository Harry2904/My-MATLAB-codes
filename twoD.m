...
dt = 360; % Time step (s)
total_time = 24 * 3600; % 1 day in seconds
t_max = 3 * total_time; % 72 hours simulation
...
unsteadiness_nd = 1; 
n = 0;
time_record = 0:dt:t_max;
outer_surface_temp = zeros(1, length(time_record));

while unsteadiness_nd >= epsilon_st && n*dt < t_max
    n = n + 1;
    ...
    T_inf_t = T_inf_avg + T_inf_amp * cos(2 * pi * n * dt / (24 * 3600));
    aS_conv = k / dy(jmax-1);
    aP_conv = aS_conv + h;
    ...
    for j = 2:jmax-1
        for i = 2:imax-1
            b(i,j) = aP_0(i,j) * T_old(i,j) + q_flux / Dy_vals(j);
        end
    end
    ...
    outer_surface_temp(n) = mean(T(:, jmax));
end

% Time in hours
time_hours = time_record / 3600;

% Plot outer surface temperature variation
figure;
plot(time_hours, outer_surface_temp, 'b', 'LineWidth', 2);
xlabel('Time (hours)');
ylabel('Outer Surface Temperature (°C)');
title('Thermal Response of Pipe Wall');
grid on;
